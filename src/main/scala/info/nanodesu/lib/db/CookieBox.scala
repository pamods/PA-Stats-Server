...
object CookieFunc {
  ...
  def epoch(f: Field[Timestamp]) = DSL.field("date_part('epoch', {0}::timestamptz)", classOf[BigInteger], f)
  def intervalInSecs(f: Field[Timestamp]) = DSL.field("date_part('epoch', {0})", classOf[BigInteger], f)
