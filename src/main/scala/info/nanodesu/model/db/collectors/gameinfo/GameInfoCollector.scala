...
case class GameInfo(startTime: Date, paVersion: String, winner: String, duration: Long, lobbyId: String)
...
trait GameInfoLoader {
	def getGameInfo(db: DSLContext, gameId: Int) = {
	  db.select(games.START_TIME, games.PA_VERSION, games.WINNER, 
	      intervalInSecs(games.END_TIME.sub(games.START_TIME).mul(int2Num(1000))), games.IDENT).
	  from(games).
